...
import os
saved_environment = os.environ
def test_import():
    import os
    import smdistributed.dataparallel.torch.torch_smddp
    # #os.environ = saved_environment
    # #print(os.environ)
    # os.environ["SMDATAPARALLEL_EC2_ENABLED"]="1"
    # os.environ["TRAINING_JOB_ARN"]="job"
    # os.environ["TRAINING_JOB_NAME"]="job"
...
def train_fashion_mnist(num_workers=2, use_gpu=False):
    breakpoint()
    torch_config = TorchConfig(backend = "smddp")

    # The main class that is exposed in the Ray Train API that users will interact with
    trainer = Trainer(backend= torch_config, num_workers=num_workers, use_gpu=use_gpu)

    
    trainer.start(initialization_hook=test_import)
